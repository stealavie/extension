...
(async () => {
    ...
    try {
        ...
        console.log(initialResponse, initialData);

        if (initialResponse.ok && initialResponse.status === 202) {
            updateStatus('Server đã nhận yêu cầu', 20, 'success');

            // Use task_id from the response
            const taskId = initialData.task_id;
            
            if (!taskId) {
                updateStatus('Lỗi: Không nhận được task_id từ server', 100, 'error');
                return;
            }

            // Poll for status updates using the task_id
            const intervalId = setInterval(async () => {
                try {
                    const statusResponse = await fetch(`${serverUrl}/status?video_id=${taskId}`, {
                        headers: {
                    ...
                    if (!contentType || !contentType.includes('application/json')) {
                        console.error('Response is not JSON:', await statusResponse.text());
                        clearInterval(intervalId);
                        updateStatus('Lỗi: Server trả về dữ liệu không hợp lệ', 100, 'error');
                        return;
                    }
                    ...
                    if (statusData.status === 'transcription_complete') {
                        ...
                        if (statusData.subtitles && Array.isArray(statusData.subtitles) && statusData.subtitles.length > 0) {
                            // Filter out invalid subtitle entries
                            ...
                            if (validSubtitles.length > 0) {
                                setupSubtitleDisplay(validSubtitles);
                                updateStatus('✨ Phụ đề đã sẵn sàng!', 100, 'success');
                            } else {
                                updateStatus('⚠️ Không có phụ đề hợp lệ', 100, 'error');
                            }
                        } else {
                            updateStatus('⚠️ Không nhận được dữ liệu phụ đề', 100, 'error');
                        }
                        ...
                        clearInterval(intervalId);
                        updateStatus(`Lỗi: ${statusData.message}`, statusData.progress || 100, 'error');
                    }
                } catch (pollError) {
